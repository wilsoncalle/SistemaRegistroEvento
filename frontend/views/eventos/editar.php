<?php // Formulario de edición de eventos ?> 
...
// Obtener ponentes actuales del evento
$query = "SELECT ponente_id FROM eventos_ponentes WHERE evento_id = ?";
$stmt = mysqli_prepare($conn, $query);
mysqli_stmt_bind_param($stmt, 'i', $evento_id);
mysqli_stmt_execute($stmt);
$resultado = mysqli_stmt_get_result($stmt);
...
while ($row = mysqli_fetch_assoc($resultado)) {
    $ponentes_actuales[] = $row['ponente_id'];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (!mysqli_stmt_execute($stmt)) {
            throw new Exception("Error al actualizar los ponentes");
        }
...
<head>
    ...
    <title>Editar Evento - Sistema de Gestión de Eventos Académicos</title>
                            ...
                            <div class="col-md-8">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="titulo" name="titulo" 
                                           value="<?php echo isset($evento['titulo']) ? htmlspecialchars($evento['titulo']) : ''; ?>" required>
                                    <div class="invalid-feedback">Por favor ingresa el título del evento.</div>
                                </div>

                                <div class="mb-3">
                                    <label for="descripcion" class="form-label">Descripción *</label>
                                    <textarea class="form-control" id="descripcion" name="descripcion" rows="4" required><?php echo isset($evento['descripcion']) ? htmlspecialchars($evento['descripcion']) : ''; ?></textarea>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="ubicacion" name="ubicacion" 
                                           value="<?php echo isset($evento['ubicacion']) ? htmlspecialchars($evento['ubicacion']) : ''; ?>" required>
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <input type="url" class="form-control" id="imagen_url" name="imagen_url" 
                                           value="<?php echo isset($evento['imagen']) && !empty($evento['imagen']) ? htmlspecialchars($evento['imagen']) : ''; ?>"
                                    ...
                                    <select class="form-select" id="categoria_id" name="categoria_id" required>
                                        ...
                                        <?php while ($categoria = mysqli_fetch_assoc($categorias)): ?>
                                            <option value="<?php echo $categoria['categoria_id']; ?>" 
                                                    <?php echo $evento['categoria_id'] == $categoria['categoria_id'] ? 'selected' : ''; ?>>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control" id="max_participantes" name="max_participantes" 
                                           value="<?php echo isset($evento['max_participantes']) ? htmlspecialchars($evento['max_participantes']) : ''; ?>" min="1" required>
                                    ...
                                    <select class="form-select" id="ponentes" name="ponentes[]" multiple>
                                        <?php while ($ponente = mysqli_fetch_assoc($ponentes)): ?>
                                            <option value="<?php echo $ponente['ponente_id']; ?>" 
                                                    <?php echo in_array($ponente['ponente_id'], $ponentes_actuales) ? 'selected' : ''; ?>>
